...
def parseHyperparamIni(filepath='Hyperparams.ini'):
    netConfig = configparser.ConfigParser(filepath)
...
class DoomGameEnv(object):
    ...
    def step(self, action):
        frameBuffer = torch.zeros(3, self.imageHeight, self.imageWidth)
        reward = 0
        is_done = False
        reward = self.game.make_action(action, self.frameskip)  # Take action for frameskip frames
